...
- Un champ de saisie est une ligne de champ : aucun contour, son filet est son seul trait, l'invalidité se lit par ce filet et son étiquette passés en rouge, et depuis le lot 4.1 le focus aussi : un trait de signal de 2 px s'allume sur le filet en 150 ms, sans anneau du navigateur.
...
- L'écran de connexion est la seule colonne centrée du système (480 px au plus, rembourrage 16 px, 96 px de retrait haut à 960 px et plus) sous la même bande de chrome de 48 px : Title « Connexion », une ligne en Small encre secondaire, deux `FormField` sur filets (email, mot de passe masqué) et « Se connecter » en signal avec `log-in` calé à droite ; ni carte, ni fond, ni bouton plein, ni logo.
- Un compte en lecture seule voit la même page sans ses verbes : « Ajouter un ouvrage », la ligne « Couverture », « Supprimer » / « Modifier », le champ « Nouvelle note » et la corbeille d'une note ne sont pas rendus, les lignes Note, Coup de coeur et Lecture redeviennent des `FieldRow`, et rien n'est jamais désactivé ; au bout du bandeau, la mention « Lecture seule » en Small encre secondaire, en bas de casse, le dit une fois, devant le bouton-icône `log-out` dont le libellé parlé cite le compte.
...
### Hierarchy
- **Title** (600, 20 px / 26 px) : titre de la fiche d'ouvrage (première ligne sous l'en-tête) et titre d'écran du formulaire (« Nouvel ouvrage », « Modifier l'ouvrage »), avec 16 px en dessous avant la première ligne de champ ; depuis le lot 4.1, le titre de l'écran de connexion (« Connexion »), suivi à 4 px de sa ligne d'explication en Small puis, 16 px plus bas, du formulaire. Non utilisé dans la liste.
...
- **Body strong** (600, 14 px / 20 px) : titre d'ouvrage dans la liste (une ligne, tronquée), libellé de tout bouton texte, y compris « Fonds » et « Fermer » dans l'en-tête, « Supprimer » et « Modifier » sous la fiche, « Garder » et « Supprimer » dans la question, « Annuler » et « Enregistrer » sous le formulaire, « Annuler », « Réessayer » et « Fermer » dans la barre d'annulation, « Ajouter la note » et son libellé de progression « Ajout… », « Remplacer la couverture » et son libellé de progression « Envoi… », « Revenir à la couverture d'origine », « EN » / « FR » au bout du bandeau, « Se connecter » et son libellé de progression « Connexion… » ; et le titre de section « Notes de lecture », seul emploi hors bouton et hors liste.
- **Small** (400, 13 px / 18 px) : seconde ligne d'un ouvrage (auteur · éditeur · année) en encre secondaire, description d'un message d'état, message en ligne de retour arrière (« Modification annulée : … ») en danger, message d'un champ invalide et alerte globale du formulaire en danger ; indication en bas du menu de tri (« Choisir à nouveau le même critère inverse l'ordre. ») et « Note ajoutée » en encre secondaire ; « Suppression annulée : … » sous les notes en danger ; « Couverture refusée : … » sous la ligne « Couverture » en danger ; depuis le lot 4.1, « Le fonds est réservé aux libraires du réseau. » sous le titre de connexion et la mention « Lecture seule » au bout du bandeau (compte lecteur, 960 px et plus ; en bas de casse, pour ne pas ressembler à un onglet) en encre secondaire, et, en danger, le message d'un champ de connexion vide (« L'email est obligatoire », « Le mot de passe est obligatoire ») et la ligne « Email ou mot de passe incorrect. Vérifiez la saisie, puis réessayez. ».
...
- **Rubric** (600, 11 px / 16 px, `letterSpacing 0.88` soit 0,08 em, capitales) : trois usages, et seulement trois : nommer une rubrique, un état, un champ. Les rubriques de navigation FONDS · LUS · NON LUS · COUPS DE COEUR ; le libellé de l'état du serveur (EN LIGNE en signal, SERVEUR INJOIGNABLE en encre secondaire, VÉRIFICATION en encre avant la première réponse), jamais la mention « Lecture seule », qui est un fait sur le compte et se compose en Small pour que rien au bout du bandeau ne ressemble à un onglet ; l'étiquette d'une ligne de champ, dans la fiche (COUVERTURE, ÉDITEUR, ANNÉE, NOTE, COUP DE COEUR, LECTURE, ÉDITIONS) comme dans le formulaire (TITRE, AUTEUR, ÉDITEUR, ANNÉE, LECTURE), le formulaire de connexion (EMAIL, MOT DE PASSE) et la section des notes (NOUVELLE NOTE), en encre secondaire (danger si le champ est invalide), sur une colonne de 112 px, à côté de la valeur et jamais au-dessus d'un titre.
...
**La règle des capitales espacées.** Les capitales espacées à 11 px sont réservées à trois emplois : les rubriques de navigation, l'état du serveur (EN LIGNE, SERVEUR INJOIGNABLE, VÉRIFICATION) et l'étiquette d'un champ dans une liste de définitions, lue ou saisie. Elles nomment une section, un état ou un champ, toujours à côté ou en ligne ; elles ne servent jamais de surtitre décoratif au-dessus d'un titre. La mention « Lecture seule » du bandeau n'en est pas : c'est un fait sur le compte, en Small encre secondaire et en bas de casse, pour qu'aucun texte du bandeau qui n'est pas une rubrique n'ait l'air d'un onglet.
...
1. **Bandeau de tête** (`chromeHeight = 48`, `minHeight`) sur `surface`, rembourrage horizontal 8 px. À 960 px et plus (`twoPaneMin`), une seule rangée : rubriques à gauche, champ de recherche au centre (`flex: 1`, rembourrage 8 px), bouton texte « Ajouter un ouvrage », marque de synchronisation, puis, derrière un filet vertical de 1 px en `rule` (`Rule` vertical, `aria-hidden`, retiré de 8 px en haut et en bas, 8 px de rembourrage à gauche, écarts de 4 px), les deux commandes de préférence : le bouton-icône de thème (44 × 44 px, `moon` en clair, `sun` en sombre, encre secondaire) et le bouton texte de langue (« EN » en français, « FR » en anglais, encre), puis, un compte étant entré (`features/preferences/ControlesPreferences.tsx`), la mention « Lecture seule » en Small encre secondaire, en bas de casse (8 px de rembourrage de côté ; seulement pour un compte qui ne peut pas écrire) et le bouton-icône `log-out` (44 × 44 px, encre secondaire) dont le libellé parlé est « Se déconnecter (email) ». « Ajouter un ouvrage » n'est pas rendu pour un compte lecteur : la recherche s'étire jusqu'à la synchronisation. En dessous de 960 px, deux rangées de 48 px : recherche (« Titre ou auteur »), bouton-icône `plus` en signal libellé « Ajouter un ouvrage » (non rendu pour un compte lecteur), synchronisation, puis le même groupe de préférences derrière son filet, en `compact` : sans la mention, avec le `log-out` ; puis les rubriques en défilement horizontal sans indicateur, seules sur leur rangée.
2. **Filet.**
...
**Écran de connexion** (`app/connexion.tsx`, `features/auth/FormulaireConnexion.tsx`) : la seule colonne centrée du système. La page est une colonne `flex: 1` sur `page` ouverte par une bande de chrome de 48 px (`chromeHeight`, `minHeight`) sur `surface`, rembourrage horizontal 8 px, dont la gauche est vide (`spacer` en `flex: 1`) et la droite porte la marque de synchronisation puis `ControlesPreferences` en `compact` (filet vertical, lune ou soleil, « EN » / « FR » ; personne n'étant entré, ni mention ni `log-out`) ; un filet la ferme. Dessous, un `ScrollView` rembourré de 16 px (`space.lg`), et de 96 px en haut à 960 px et plus (`RETRAIT_HAUT_LARGE = 3 * space.xxl`, décidé par `useWindowDimensions` sur `twoPaneMin`) pour que le formulaire seul ne soit pas collé à la bande, dont le contenu est une colonne de 480 px au plus (`LARGEUR_COLONNE`), `width: 100%`, centrée par `alignSelf: 'center'` : à 1440 px elle court de 480 à 960 px. Dans la colonne, le titre « Connexion » en Title, 4 px dessous « Le fonds est réservé aux libraires du réseau. » en Small encre secondaire, 16 px dessous le formulaire : une colonne à écart 8 px de deux `FormField` de 44 px sur filets (« Email », `inputMode="email"`, `autoComplete="email"`, texte indicatif « prenom@boutique.fr », `autoFocus` ; « Mot de passe », `secureTextEntry`, `autoComplete="current-password"`, texte indicatif « Votre mot de passe », Entrée soumet), même étiquette de 112 px et même colonne de valeur à 152 px que la fiche ; s'il y a lieu, la ligne d'alerte globale en Small danger `role="alert"` 12 px sous les champs ; puis la rangée d'actions calée à droite, 16 px dessous, réduite à « Se connecter » en signal avec `log-in`, relibellé « Connexion… » et désactivé à 0,5 pendant la requête. Aucune carte, aucun fond, aucun bouton plein, aucun logo : la porte est faite des matériaux de la page. On y arrive par redirection (`app/(fonds)/_layout.tsx` : rien n'est dessiné tant que la session stockée se lit, puis `/connexion?vers=<chemin>` quand le serveur exige un jeton et que personne n'est entré) et on en repart vers `vers` (un chemin interne, jamais `/connexion`) ou `/` dès que la session est `connecte`. Un jeton renouvelé en silence (`services/api/intercepteurAuth.ts`, un seul rafraîchissement pour des 401 simultanés) ne change aucun écran : l'expiration n'est jamais visible.
...
- **Source (`features/sync/MarqueServeur.tsx`, `useEtatServeur.ts`) :** depuis le lot 4.1 la marque dit ce qui a été mesuré, pas ce qui est espéré : une sonde `/health` (`lireExigenceAuth`, React Query, `refetchInterval` 30 s, jamais en arrière-plan, sans nouvel essai) donne trois états : `joignable` → `online` « En ligne » / « Online » ; `injoignable` → `offline` « Serveur injoignable » / « Server unreachable » ; `inconnu`, avant la première réponse → `pending` « Vérification » / « Checking ». `conflict` reste défini par `SyncMark` sans producteur. La même `MarqueServeur` remplace la marque littérale dans `BandeauFonds`, dans l'en-tête écran de `CadreOuvrage` et dans `app/connexion.tsx` ; aucun écran n'écrit plus `status="online"`.
...
- **Focus :** un trait de signal de 2 px (`TRAIT_FOCUS`), posé en absolu sur toute la largeur de la ligne, à cheval sur le filet (`bottom: -hairline`), `aria-hidden`, qui passe d'opacité 0 à 1 en 150 ms (`transitionEtat`) quand le champ prend le focus et s'éteint à la perte ; le contour du navigateur est retiré sur ce `TextInput` (`sansContourFocus`, `outlineWidth: 0`, web) pour ne pas doubler le trait ; curseur et sélection en signal (`selectionColor`). L'anneau global reste pour les boutons et les champs sans filet propre (recherche, note multiligne). Le filet dit tout : `rule` au repos, signal sous le focus, danger en erreur (le trait de focus se pose alors sur le filet rouge).
- **Error :** quand un message est fourni, le filet passe de `rule` à `danger` et l'étiquette de l'encre secondaire à `danger`, en 150 ms (`transitionEtat`) ; un message en Small danger, `role="alert"`, apparaît 4 px sous la ligne, retiré de 152 px pour tomber sur la colonne des valeurs. Le texte tapé reste en encre ; aucune icône, aucun fond, aucun contour. Validation à la perte de focus (`mode: 'onBlur'`), et les erreurs du serveur sont réparties sur les champs concernés par le même mécanisme. Sur l'écran de connexion, la validation se fait à « Se connecter » (`creerIdentifiantsSchema(t.connexion)`), un message par champ vide, et un refus du serveur n'est pas réparti : il écrit la ligne globale « Email ou mot de passe incorrect. Vérifiez la saisie, puis réessayez. » 12 px sous le formulaire.
...
- **Saisie :** `TextAreaField` « Nouvelle note » ; puis la rangée d'action (« Note ajoutée » en `role="status"`, `notes-status` ; « Ajouter la note »). Après un succès, le champ se vide et le statut reste jusqu'à la frappe suivante. Pour un compte lecteur (`lectureSeule`), ni le champ ni la rangée ne sont rendus : la liste suit directement l'en-tête.
- **Liste :** `role="list"` étiquetée « Notes de lecture », filet en haut, notes en `NoteRow` ; vide, chargement et erreur passent par le message d'état et le squelette. Pour un compte lecteur, l'état vide change de voix : `book-open` à 24 px, « Aucune note de lecture », « Aucun libraire n'a encore conseillé cet ouvrage. » (`t.notes.videLecture` ; « No bookseller has recommended this book yet. »), jamais la plume `edit-3` ni l'invitation à écrire (« La première note dit à qui conseiller cet ouvrage. »).
...
- **Méta-ligne (`note-meta`) :** 44 px minimum, horodatage en Figure encre secondaire à gauche, bouton-icône `trash-2` (44 × 44, encre secondaire, `aria-label` « Supprimer la note du … ») calé à droite. Pour un compte lecteur, `onDelete` est absent : la méta-ligne garde ses 44 px et son horodatage, sans corbeille, et la question ne peut pas s'ouvrir.
...
- **Case d'icône :** 20 px de large (`layout.iconCase`), centrée, toujours rendue même vide ; quand une icône est fournie (le coeur du coup de coeur à « Oui », dans la fiche lue par un compte lecteur), elle est en Feather 16 px ton signal.
...
### Préférences et session du bandeau (ControlesPreferences)
Les deux réglages qu'une boutique change à l'ouverture et à la fermeture, puis, une fois un compte entré, ce qu'il faut savoir de la session et la porte de sortie, posés au bout du bandeau, jamais dans un menu ni une page de réglages ni un menu de compte (`features/preferences/ControlesPreferences.tsx`, extrait de `BandeauFonds` au lot 4.1 et partagé avec l'écran de connexion).
...
- **Lecture seule (`session-mention`) :** une fois un compte entré et seulement s'il ne peut pas écrire (`useSession().peutEcrire` faux), la mention « Lecture seule » en Small encre secondaire et en bas de casse, 8 px de rembourrage de côté, après « EN » / « FR » ; un texte, pas un bouton, et pas une capitale espacée, pour que rien au bout du bandeau ne ressemble à un onglet ; masquée en `compact` (sous 960 px et sur l'écran de connexion), la fiche disant alors la chose par l'absence de ses verbes.
- **Déconnexion (`session-signout`) :** `IconButton` de 44 × 44 px, `log-out`, encre secondaire, dernier élément du bandeau, rendu dès qu'un compte est entré ; son libellé parlé est « Se déconnecter (email) » : l'icône est la porte, le libellé cite le compte. Presser vide la session ; le groupe de routes ramène alors à `/connexion`.
- **Groupe :** une rangée (`styles.groupe`) ouverte par un filet vertical de 1 px en `rule` (`Rule orientation="vertical"`, `aria-hidden`), retirée de 8 px en haut et en bas (le filet fait 32 px dans la bande de 48), 8 px de rembourrage à gauche, 4 px entre le filet, la lune et « EN » ; le filet sépare ce qui est réglage de ce qui est état.
- **Place :** à 960 px et plus, après la marque de synchronisation, dans l'ordre thème, langue, puis, un compte étant entré, la mention de lecture seule et la déconnexion ; en dessous, au bout de la première rangée, après la recherche, le bouton-icône `plus` (non rendu pour un compte lecteur) et la synchronisation, en `compact` sans la mention ; les rubriques gardent la seconde rangée entière. Sur l'écran de connexion, le même groupe en `compact` ferme la bande, après la synchronisation, sans `log-out` puisque personne n'est entré.

### Écran de connexion (FormulaireConnexion)
La porte du fonds, faite des matériaux de la page ; géométrie dans Layout › Écran de connexion.
- **Bande (`login-band`) :** 48 px sur `surface`, rembourrage 8 px, vide à gauche, marque de synchronisation puis préférences `compact` à droite, filet dessous ; la même bande que le fonds, sans rubriques, sans recherche, sans action.
- **Colonne (`login-column`) :** 480 px au plus, `width: 100%`, centrée, rembourrage 16 px (96 px en haut à 960 px et plus, `RETRAIT_HAUT_LARGE`) ; Title « Connexion », puis « Le fonds est réservé aux libraires du réseau. » en Small encre secondaire (`login-explication`, 4 px dessus, 16 px dessous).
- **Champs :** deux `FormField` à écart 8 px, « Email » (`inputMode="email"`, `autoCapitalize="none"`, `autoComplete="email"`, texte indicatif « prenom@boutique.fr ») et « Mot de passe » (`secureTextEntry`, `autoComplete="current-password"`, texte indicatif « Votre mot de passe ») ; le premier a le focus au montage (`autoFocus`) et son filet s'allume en signal dès l'arrivée (trait de focus de `FormField`, sans anneau) ; Entrée dans le second soumet (`onSubmitEditing`).
- **Erreurs :** validation à la soumission (`creerIdentifiantsSchema(t.connexion)`) ; un champ vide reçoit son message en Small danger `role="alert"` sous sa ligne (« L'email est obligatoire », « Le mot de passe est obligatoire »), filet et étiquette en danger ; un refus du serveur n'est pas réparti sur les champs mais écrit la ligne globale (`login-alert`) « Email ou mot de passe incorrect. Vérifiez la saisie, puis réessayez. » en Small danger `role="alert"`, 12 px sous les champs (`messagePourErreur`, `erreurs.identifiantsInvalides` : titre puis détail).
- **Action :** une rangée calée à droite, 16 px sous les champs, réduite à « Se connecter » en signal avec `log-in` ; désactivé à 0,5 et relibellé « Connexion… » pendant la requête. Pas d'« Annuler », pas de « Mot de passe oublié », pas de bouton plein.
- **Entrée et sortie :** on arrive par redirection depuis le groupe `(fonds)` avec `?vers=<chemin>` ; dès que la session est `connecte`, `Redirect` renvoie vers `vers` (un chemin interne, jamais `/connexion`) ou `/`. Rien n'est dessiné tant que la session stockée se lit : ni fonds ni porte ne clignotent. Un jeton expiré se renouvelle en silence ; l'écran ne change pas.
...
### Named Rules
**La règle du champ sur filet.** Un champ de saisie est une ligne de champ dont la valeur se tape : même étiquette de 112 px, même filet, même hauteur de 44 px, et le texte tapé commence à 152 px, sur la colonne des valeurs de la fiche. Aucun contour, aucun fond, aucun rayon ne distingue « à lire » de « à saisir » ; seul le curseur le fait. Le focus lui-même vit sur le filet : un trait de signal de 2 px qui s'allume en 150 ms, jamais un anneau autour du champ.
...
**La règle de la porte en chrome.** L'écran de connexion est fait des matériaux de la page : la bande de chrome de 48 px sur `surface` avec sa marque de synchronisation et ses préférences, un filet, puis une colonne centrée de 480 px au plus (rembourrage 16 px, 96 px en haut à 960 px et plus) où le Title, une ligne en Small encre secondaire, deux lignes de champ sur filets et un bouton texte en signal se suivent aux pas de la fiche (4 / 16 / 8 / 16 px). C'est la seule colonne centrée du système et le seul formulaire à un bouton ; ni carte, ni fond, ni logo, ni bouton plein, ni « Annuler ».

**La règle de la session au bout du bandeau.** Ce qu'il faut savoir d'un compte tient au bout du groupe des préférences, après « EN » / « FR » : la mention « Lecture seule » en Small encre secondaire, en bas de casse, quand le compte ne peut pas écrire (un texte, pas un bouton ; absente en `compact`), puis la déconnexion en bouton-icône `log-out` de 44 × 44 px dont le libellé parlé cite le compte (« Se déconnecter (email) »). Ni avatar, ni nom affiché, ni menu de compte : le nom vit dans le libellé parlé, le fait sur le compte en Small, et les capitales espacées restent aux rubriques et à l'état du serveur.

**La règle des verbes absents.** Un compte en lecture seule voit la même page à laquelle on a retiré ses verbes : « Ajouter un ouvrage » et son `plus`, la ligne « Couverture », les étoiles et les interrupteurs, « Supprimer » / « Modifier », le champ « Nouvelle note », « Ajouter la note » et la corbeille d'une note ne sont pas rendus. Les mêmes lignes se lisent en valeurs (`FieldRow` : « 1/5 » ou « Sans note » en encre secondaire, « Oui » avec le coeur en signal ou « Non », « Lu » / « Non lu »), au même pas de 52 px, et la section des notes suit « Éditions » aux mêmes 24 px. Rien n'est grisé, rien n'est désactivé, rien n'explique : l'absence est la règle, et la mention du bandeau la dit une fois. Vide, la section des notes lui montre `book-open` et un fait (« Aucun libraire n'a encore conseillé cet ouvrage. »), pas la plume ni l'invitation à écrire.
...
- **Do** composer l'écran de connexion comme une page du fonds sans sa liste : bande de chrome de 48 px (synchronisation, préférences `compact`), filet, colonne centrée de 480 px au plus rembourrée de 16 px (96 px en haut à 960 px et plus), Title « Connexion », Small encre secondaire à 4 px, deux `FormField` (email, `secureTextEntry`) à 8 px d'écart 16 px plus bas, alerte globale en Small danger `role="alert"` à 12 px, « Se connecter » en signal avec `log-in` calé à droite à 16 px, relibellé « Connexion… » et désactivé à 0,5 pendant la requête ; focus sur l'email au montage.
- **Do** poser la session au bout du bandeau après « EN » / « FR » : la mention « Lecture seule » en Small encre secondaire, en bas de casse (8 px de côté), pour un compte qui ne peut pas écrire, masquée en `compact`, puis le bouton-icône `log-out` de 44 × 44 px dont le libellé parlé est « Se déconnecter (email) ».
- **Do** retirer les verbes plutôt que les griser pour un compte lecteur (`useSession().peutEcrire`) : ne pas rendre « Ajouter un ouvrage » ni son `plus`, la ligne « Couverture », la rangée « Supprimer » / « Modifier », le champ « Nouvelle note », « Ajouter la note » et la corbeille d'une note ; rendre « Note », « Coup de coeur » et « Lecture » en `FieldRow` (« Sans note » en `muted`, coeur en signal à « Oui ») ; état vide des notes en `book-open` avec « Aucun libraire n'a encore conseillé cet ouvrage. ».
- **Do** ne rien dessiner tant que la session stockée se lit, rediriger vers `/connexion?vers=<chemin>` quand le serveur exige un jeton et que personne n'est entré, et renouveler un jeton expiré en silence (un seul rafraîchissement pour des 401 simultanés) sans changer d'écran.
- **Do** dessiner le focus d'une ligne de saisie (`FormField`) sur son filet : trait de signal de 2 px à cheval sur le filet (`bottom: -hairline`), opacité 0 → 1 en 150 ms (`transitionEtat`), `aria-hidden`, contour du navigateur retiré sur ce seul `TextInput` (`sansContourFocus`, web) ; garder l'anneau global pour tout ce qui n'a pas de filet.
- **Do** nourrir la marque du bandeau par `MarqueServeur` (`features/sync`), jamais par un littéral : sonde `/health` toutes les 30 s (`useEtatServeur`, React Query, pas en arrière-plan, sans nouvel essai) ; `online` « En ligne », `offline` « Serveur injoignable », `pending` « Vérification » avant la première réponse ; la même marque dans `BandeauFonds`, `CadreOuvrage` et l'écran de connexion.
...
- **Don't** remplacer un bouton texte par un bouton-icône quand un libellé tient ; la corbeille d'une note, la bascule de thème, le `plus` sous 960 px et la déconnexion sont les seuls boutons-icônes, et leur `aria-label` cite l'objet ou dit l'action.
...
- **Don't** mettre l'écran de connexion dans une carte, sur un fond, derrière un logo ou sous un bouton plein ; c'est la bande de chrome, un filet, une colonne centrée de 480 px et des lignes de champ sur filets.
- **Don't** griser, désactiver ou expliquer une action interdite à un compte lecteur ; elle n'est pas rendue, et la même ligne se lit en valeur.
- **Don't** afficher un nom, un avatar ou un menu de compte dans le bandeau ; la session tient en une mention Small « Lecture seule » et un bouton-icône `log-out` dont le libellé parlé cite le compte.
- **Don't** laisser l'anneau du navigateur entourer un `FormField` ni doubler le trait de focus ; sur une ligne de saisie, le focus est le filet passé en signal, et rien d'autre ne change.
- **Don't** écrire l'état du serveur en dur (`status="online"`) ni composer la mention « Lecture seule » en capitales espacées ; le bandeau dit ce qui a été mesuré, et seuls les onglets et l'état du serveur y sont en Rubric.
